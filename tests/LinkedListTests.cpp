...
thenCollectionContainsValues(collection, { 23 , 16 });
  BOOST_CHECK_EQUAL(collection.getSize(), 2);
}

//dopisany test
BOOST_AUTO_TEST_CASE_TEMPLATE(GivenCollectionWithOneElement_WhenErasingOneElement_ThenBeginEqualsEnd,
        T,
        TestedTypes)
{
...
BOOST_CHECK(collection.isEmpty());
BOOST_CHECK(collection.head == collection.tail);
}
//dopisany test 2
BOOST_AUTO_TEST_CASE_TEMPLATE(GivenCollectionWithTwoElement_WhenErasingOneElement_ThenBeginEqualsEnd,
        T,
        TestedTypes)
{
LinearCollection<T> collection;
...
auto it = begin(collection);
collection.erase(it);
auto it2 = begin(collection);
collection.erase(it2);


//thenConstructedObjectsCountWas<T>(6);
thenCollectionContainsValues(collection, {4});
BOOST_CHECK(!(collection.isEmpty()));
BOOST_CHECK(collection.head == collection.tail);
BOOST_CHECK(++begin(collection)==end(collection));
}
